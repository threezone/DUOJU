...
namespace Duoju.DAO.Utilities
{
    public sealed class EntityReader
    {
        ...
        public static DataTable ToDataTable<T>(List<T> list, params string[] propertyName) where T : new()
        {
        ...
        private static List<T> GetComplexEntities<T>(DataTable dataTable) where T : new()
        {
            ...
            foreach (DataRow row in dataTable.Rows)
            {
                ...
                {
                    //如果对应的列在存在于DataRow中，则继续，若不存在，说明该业务不查该字段，不做任何操作
        ...
        private static List<T> GetComplexEntities<T>(DbDataReader reader) where T : new()
        {
            ...
            while (reader.Read())
            {
                ...
                {
                    //如果对应的列在存在于DataRow中，则继续，若不存在，说明该业务不查该字段，不做任何操作
                    if (reader.GetSchemaTable().Columns.Contains(item.Key))
                    {
                        //如果对应的属性名出现在数据源的列中则获取值并设置给对应的属性  
