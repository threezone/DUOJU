namespace DUOJU.Domain
{
    public class CommonSettings
    {
        public const string OPERATE_FAIL = "fail";

        public const string OPERATE_SUCCESS = "success";

        public const string TIPS_FAIL = "操作失败！";

        public const string TIPS_SUCCESS = "操作成功！";


        public const int OPERATOR_SYSTEM_ID = 0;

        public const double USERREGISTER_DEFAULT_COIN_COUNT = 5;

        public const string USERACCOUNT_WECHAT_FORMAT = "wechat_{0}";


        public const string DUOJUWECHAT_TOKEN = "suibianxiexie";

        public const string DUOJUWECHAT_APPID = "wx2b8e67341ef40666";

        public const string DUOJUWECHAT_APPSECRET = "5e1c314ef6d160d2fbfa4d8bc0beef2d";


        public const int IDENTIFIEREXPIRESTIME_DAY_PARTY = 7;

        public const string IDENTIFIERSETTINGCODE_PARTYID = "partyid";
    }
}
